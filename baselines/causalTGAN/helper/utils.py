...
from baselines.causalTGAN.dataset import DataTransformer, GeneralTransformer, PlainTransformer
from baselines.causalTGAN.helper.constant import HOUSING_CATEGORY,HELOC_CATEGORY,EARLY_DIAB,BEIJING_CATEGORY,KINGS_CATEGORY, LOAN_CATEGORY, CABS_CATEGORY, ADULT_CATEGORY, NEWS_CATEGORY, CENSUS_CATEGORY, CREDIT_CATEGORY
...
def get_discrete_cols(data, data_name):
    ...
    elif data_name == "beijing":
        discrete_cols = BEIJING_CATEGORY
    elif data_name == "early_diab":
        discrete_cols = EARLY_DIAB
    elif data_name == "heloc":
        discrete_cols = HELOC_CATEGORY
    elif data_name == "housing":
        discrete_cols = HOUSING_CATEGORY
    elif data_name == "loan":
        discrete_cols = LOAN_CATEGORY
...
def load_mod_graph(dataname):
    ...
    NODES.sort()
    if dataname =="adult":
        columns.remove('education.num')
        
    map_node_column = {i: col for i,col in zip(NODES,columns)}
    ...
    for edge in dag_data['edges']:
        ...
        GRAPH[node].append(PA)


    MAPPED_GRAPH = []

    for k,v in GRAPH.items():
        MAPPED_GRAPH.append([map_node_column[k], [map_node_column[n] for n in v]])

    if dataname =="adult":
        MAPPED_GRAPH.append(['education.num',[]])
...
def load_data_graph(data_name, graph_path=None):
    ...
    causal_graph = load_mod_graph(data_name)

    data_path = os.path.join(data_folder, 'train.csv')
    #data = pd.read_csv(data_path).iloc[:, 1:] changed adult age col missed
