...
#from rdt.transformers.categorical import OneHotEncoder, LabelEncoder
...
class DataTransformer(object):
    ...
    def _fit_discrete(self, column_name, raw_column_data):
        ...
        ohe = OneHotEncoder(sparse_output=False) #LabelEncoder()
        print(raw_column_data,column_name)
        #ohe.fit(data=pd.DataFrame(raw_column_data,columns=[column_name]),column=column_name)
        ohe.fit(pd.DataFrame(raw_column_data))
        #num_categories = len(ohe.dummies)
        #num_categories = 1
        num_categories = ohe.categories_[0].shape[0]
    ...
    def _transform_discrete(self, column_transform_info, raw_column_data):
        ...
        print(ohe.transform(raw_column_data).shape)
    ...
    def transform(self, raw_data):
        ...
        for column_transform_info in self._column_transform_info_list:
            ...
            if column_transform_info.column_type == "continuous":
                ...
                print(f"Col name {column_transform_info.column_name}")
                print(column_data_list[-1].shape)
            else:
                assert column_transform_info.column_type == "discrete"
                column_data_list += self._transform_discrete(
                    column_transform_info, pd.DataFrame(column_data,columns=[column_transform_info.column_name]))
            out_dimensions.append(column_transform_info.output_dimensions)
        print(f"HELLLLOOOO {[column_data_list[i].shape for i in range(len(column_data_list))]}")
        return np.concatenate(column_data_list, axis=1).astype(float), out_dimensions

    def _inverse_transform_continuous(self, column_transform_info, column_data, sigmas, st):
        gm = column_transform_info.transform
        print(column_transform_info)
        valid_component_indicator = column_transform_info.transform_aux
    ...
    def _inverse_transform_discrete(self, column_transform_info, column_data):
        ...
        print(column_transform_info)
        #print(column_data)
        return ohe.inverse_transform(pd.DataFrame(column_data)).flatten()#ohe.reverse_transform(column_data).flatten()#pd.DataFrame(column_data,columns=[column_transform_info.column_name]))
    ...
    def inverse_transform(self, data, sigmas=None):
        ...
        for column_transform_info in self._column_transform_info_list:
            ...
            if column_transform_info.column_type == 'continuous':
                ...
                print(recovered_column_data.shape)
            else:
                assert column_transform_info.column_type == 'discrete'
...
class GeneralTransformer(object):
    ...
    def _fit_discrete(self, column_name, raw_column_data):
        ...
        num_categories = len(ohe.dummies) #only for ONEHOTENCO!
        #num_categories  =1
